...
namespace HighlightPlus
{
    ...
    public partial class HighlightEffect : MonoBehaviour
    {
        ...
        [Tooltip(
            "Custom sorting priority for the highlight effect (useful to control which effects render on top of others")]
        ...
        /// </summary>
        [Tooltip(
            "Bakes skinned mesh to leverage GPU instancing when using outline/outer glow with mesh-based rendering. Reduces draw calls significantly on skinned meshes.")]
        ...
        [Tooltip("Minimum width when the constant width option is not used")] [Range(0, 1)]
        public float minimumWidth;

        [Tooltip("Mask to include or exclude certain submeshes. By default, all submeshes are included.")]
        public int subMeshMask = -1;

        [Range(0, 1)] [Tooltip("Intensity of the overlay effect. A value of 0 disables the overlay completely.")]
        ...
        [Range(0, 1)] public float overlayMinIntensity = 0.5f;
        ...
        [Tooltip("Optional overlay texture.")] public Texture2D overlayTexture;
        ...
        [Range(0, 1)] [Tooltip("Intensity of the outline. A value of 0 disables the outline completely.")]
        ...
        [Range(1, 3)] public int outlineBlurPasses = 2;
        ...
        [Range(1, 8)] [Tooltip("Reduces the quality of the outline but improves performance a bit.")]
        ...
        [Range(0, 5)] [Tooltip("The intensity of the outer glow effect. A value of 0 disables the glow completely.")]
        public float glow;
        ...
        [Range(1, 8)] [Tooltip("Reduces the quality of the glow but improves performance a bit.")]
        public int glowDownsampling = 2;

        [ColorUsage(true, true)] public Color glowHQColor = new(0.64f, 1f, 0f, 1f);
        ...
        [Range(0, 5f)] [Tooltip("The intensity of the inner glow effect. A value of 0 disables the glow completely.")]
        public float innerGlow;

        [Range(0, 2)] public float innerGlowWidth = 1f;
        ...
        [Tooltip(
            "This option clears the stencil buffer after rendering the see-through effect which results in correct rendering order and supports other stencil-based effects that render afterwards.")]
        ...
        private struct ModelMaterials
        {
            ...
            public Material[] fxMatMask,
                ...
                fxMatGlow,
                fxMatSolidColor,
                fxMatSeeThroughInner,
                fxMatSeeThroughBorder,
                fxMatOverlay,
                fxMatInnerGlow;
        ...
        [SerializeField] [HideInInspector] private ModelMaterials[] rms;
        [SerializeField] [HideInInspector] private int rmsCount;
        ...
        /// </summary>
        [NonSerialized] public bool staticChildren;
#endif
        ...
        /// </summary>
        [NonSerialized] public float highlightStartTime;

        /// <summary>
        /// Time in which the target fx started
        /// </summary>
        [NonSerialized] public float targetFxStartTime;
        ...
        /// </summary>
        [NonSerialized] public bool spriteMode;

        [NonSerialized] public HighlightProfile previousSettings;
        ...
        private Material fxMatGlowTemplate, fxMatInnerGlow, fxMatOutlineTemplate, fxMatTarget;
        private Material fxMatComposeGlow, fxMatComposeOutline, fxMatBlurGlow, fxMatBlurOutline;
        ...
        [NonSerialized] public static List<HighlightEffect> effects = new();
        ...
        private bool useSmoothGlow, useSmoothOutline, useSmoothBlend;
        ...
        private class PerCameraOcclusionData
        {
            ...
            public readonly List<Renderer> cachedOccluders = new();
            public Collider cachedOccluderCollider;
        }

        private readonly Dictionary<Camera, PerCameraOcclusionData> perCameraOcclusionData = new();

        private MaterialPropertyBlock glowPropertyBlock, outlinePropertyBlock;
        private static readonly List<Vector4> matDataDirection = new();
        private static readonly List<Vector4> matDataGlow = new();
        private static readonly List<Vector4> matDataColor = new();
        ...
        [NonSerialized] public string lastRegExError;
        ...
        {
            if (rms == null || !isInitialized) Init();
        ...
        private void Init()
        {
            ...
            if (corners == null || corners.Length != 8) corners = new Vector4[8];

            InitCommandBuffer();
            if (quadMesh == null) BuildQuad();

            if (cubeMesh == null) BuildCube();
            ...
            if (useGPUInstancing)
            {
                if (glowPropertyBlock == null) glowPropertyBlock = new MaterialPropertyBlock();
            ...
            if (profileSync && profile != null) profile.Load(this);
        ...
        private void OnDisable()
        {
            ...
            if (effects != null)
            {
                var k = effects.IndexOf(this);
                if (k >= 0) effects.RemoveAt(k);
            }
        }
        ...
        {
            if (mm == null) return;
            for (var k = 0; k < mm.Length; k++) DestroyMaterial(mm[k]);
        }
        ...
        private void OnDestroy()
        {
            ...
            if (combinedMeshes.ContainsKey(combinedMeshesHashId)) combinedMeshes.Remove(combinedMeshesHashId);
        ...
        {
            if (profile != null) profile.Load(this);
        }
        ...
        {
            if (profile != null) profile.Save(this);
        }
        ...
        {
            if (discardCachedMeshes) RefreshCachedMeshes();

            InitIfNeeded();
            if (enabled) SetupMaterial();
        }

        private RenderTargetIdentifier colorAttachmentBuffer, depthAttachmentBuffer;
        ...
        public CommandBuffer BuildCommandBuffer(Camera cam, RenderTargetIdentifier colorAttachmentBuffer,
            RenderTargetIdentifier depthAttachmentBuffer, bool clearStencil)
        {
        ...
        {
            if (colorAttachmentBuffer == 0) colorAttachmentBuffer = BuiltinRenderTextureType.CameraTarget;

            if (depthAttachmentBuffer == 0) depthAttachmentBuffer = BuiltinRenderTextureType.CameraTarget;
            ...
            var independentFullScreenNotExecuted = true;
            if (clearStencil)
            {
                ConfigureOutput();
            ...
            var seeThroughReal = usesSeeThrough;
            if (seeThroughReal)
            {
                ConfigureOutput();
            ...
            var showOverlay = hitActive || overlayMode == OverlayMode.Always;
            if (!_highlighted && !seeThroughReal && !showOverlay) return;
            ...
            // Apply effect
            var glowReal = _highlighted ? glow : 0;
            if (fxMatMask == null)
                return;
            ...
            var smoothGlowVisibility = glowVisibility;
            var smoothOutlineVisibility = outlineVisibility;

            // First create masks
            var aspect = cam.aspect;
            var somePartVisible = false;

            for (var k = 0; k < rmsCount; k++)
            {
                rms[k].render = false;

                var t = rms[k].transform;
                if (t == null)
                    continue;

                if (rms[k].isSkinnedMesh && shouldBakeSkinnedMesh)
                {
                    var smr = (SkinnedMeshRenderer)rms[k].renderer;
                    if (rms[k].bakedSkinnedMesh == null) rms[k].bakedSkinnedMesh = new Mesh();
                ...
                if (rms[k].isCombined) rms[k].renderingMatrix = t.localToWorldMatrix;
                ...
                if (outlineIndependent)
                {
                    ...
                    else if (outline > 0 || glow > 0)
                    {
                        ...
                        if (glow > 0) width = Mathf.Max(width, glowWidth);

                        for (var l = 0; l < mesh.subMeshCount; l++)
                        {
                            if (((1 << l) & subMeshMask) == 0) continue;
                            if (outlineQuality.UsesMultipleOffsets())
                            {
                                for (var o = outlineOffsetsMin; o <= outlineOffsetsMax; o++)
                                {
                                    var direction = offsets[o] * (width / 100f);
                                    direction.y *= aspect;
                                    cbHighlight.SetGlobalVector(ShaderParams.OutlineDirection, direction);
                                    if (rms[k].isCombined)
                                        cbHighlight.DrawMesh(rms[k].mesh, rms[k].renderingMatrix,
                                            rms[k].fxMatOutline[l], l, 1);
                                    else
                            ...
                            else
                            {
                                ...
                                if (rms[k].isCombined)
                                    cbHighlight.DrawMesh(rms[k].mesh, rms[k].renderingMatrix, rms[k].fxMatOutline[l], l,
                                        1);
                                else
                                    cbHighlight.DrawRenderer(rms[k].renderer, rms[k].fxMatOutline[l], l, 1);
                            }
            ...
            var renderMaskOnTop = _highlighted && ((outline > 0 && smoothOutlineVisibility != Visibility.Normal) ||
                                                   (glow > 0 && smoothGlowVisibility != Visibility.Normal) ||
                                                   (innerGlow > 0 && innerGlowVisibility != Visibility.Normal));
            renderMaskOnTop =
            ...
            if (fading != FadingState.NoFading)
            {
                ...
                else if (fadeOutDuration > 0)
                {
                    ...
                    if (fadeGroup < 0f)
                    {
                        ...
                        if (OnObjectHighlightEnd != null) OnObjectHighlightEnd(gameObject);
            ...
            if (glowQuality == QualityLevel.High)
                glowReal *= 0.25f;
            else if (glowQuality == QualityLevel.Medium) glowReal *= 0.5f;

            var targetEffectRendered = false;
            var usesSeeThroughBorder = seeThroughBorder * seeThroughBorderWidth > 0;

            var enclosingBounds = new Bounds();
            ...
            for (var k = 0; k < rmsCount; k++)
            {
                ...
                fade = fadeGroup;
                // Distance fade
                if (cameraDistanceFade) fade *= ComputeCameraDistanceFade(rms[k].transform.position, cam.transform);
                ...
                if (_highlighted || showOverlay)
                {
                    ...
                    var overlayMinIntensity = this.overlayMinIntensity;
                    var overlayBlending = this.overlayBlending;

                    var innerGlowColorA = innerGlowColor;
                    var innerGlow = this.innerGlow;

                    if (hitActive)
                    {
                        overlayColor.a = _highlighted ? overlay : 0;
                        innerGlowColorA.a = _highlighted ? innerGlow : 0;
                        var t = hitFadeOutDuration > 0 ? (now - hitStartTime) / hitFadeOutDuration : 1f;
                        if (t >= 1f)
                        {
                            hitActive = false;
                        }
                        else
                        {
                            ...
                            {
                                var lerpToCurrentInnerGlow = _highlighted && innerGlow > 0;
                                innerGlowColorA = lerpToCurrentInnerGlow
                                    ? Color.Lerp(hitColor, innerGlowColor, t)
                                    : hitColor;
                            ...
                            {
                                var lerpToCurrentOverlay = _highlighted && overlay > 0;
                    ...
                    for (var l = 0; l < mesh.subMeshCount; l++)
                    {
                        ...
                        if (overlayColor.a > 0)
                        {
                            var fxMat = rms[k].fxMatOverlay[l];
                            fxMat.SetColor(ShaderParams.OverlayColor, overlayColor);
                            fxMat.SetVector(ShaderParams.OverlayData,
                                new Vector4(overlayAnimationSpeed, overlayMinIntensity, overlayBlending,
                                    overlayTextureScale));
                            if (hitActive && hitFxMode == HitFxMode.LocalHit)
                            {
                                fxMat.SetVector(ShaderParams.OverlayHitPosData,
                                    new Vector4(hitPosition.x, hitPosition.y, hitPosition.z, hitRadius));
                ...
                var allowGPUInstancing = useGPUInstancing && (shouldBakeSkinnedMesh || !rms[k].isSkinnedMesh);
                ...
                for (var l = 0; l < mesh.subMeshCount; l++)
                {
                    ...
                    if (glow > 0 && glowQuality != QualityLevel.Highest)
                    {
                        ...
                        for (var glowPass = 0; glowPass < glowPasses.Length; glowPass++)
                            if (glowQuality.UsesMultipleOffsets())
                            {
                                for (var o = glowOffsetsMin; o <= glowOffsetsMax; o++)
                                {
                                    var direction = offsets[o];
                                    direction.y *= aspect;
                                    var dataColor = glowPasses[glowPass].color;
                                    var dataGlow = new Vector4(fade * glowReal * glowPasses[glowPass].alpha,
                                        glowPasses[glowPass].offset * glowWidth / 100f, glowMagicNumber1,
                                        glowMagicNumber2);
                                    if (allowGPUInstancing)
                                    {
                                        ...
                                        matDataColor.Add(
                                            new Vector4(dataColor.r, dataColor.g, dataColor.b, dataColor.a));
                                    }
                                    else
                                    {
                            ...
                            {
                                var dataGlow = new Vector4(fade * glowReal * glowPasses[glowPass].alpha,
                                    glowPasses[glowPass].offset * glowWidth / 100f, glowMagicNumber1, glowMagicNumber2);
                                var dataColor = glowPasses[glowPass].color;
                                if (allowGPUInstancing)
                                {
                                ...
                                {
                                    var matIndex = glowPass * 8;
                                    cbHighlight.SetGlobalColor(ShaderParams.GlowColor, dataColor);
                                    cbHighlight.SetGlobalVector(ShaderParams.Glow, dataGlow);
                                    cbHighlight.SetGlobalVector(ShaderParams.GlowDirection, Vector4.zero);
                                    if (rms[k].isCombined)
                                        cbHighlight.DrawMesh(rms[k].mesh, rms[k].renderingMatrix, rms[k].fxMatGlow[l],
                                            l);
                                    else
                                        cbHighlight.DrawRenderer(rms[k].renderer, rms[k].fxMatGlow[l], l);
                                }
                        ...
                        if (allowGPUInstancing)
                        {
                            var instanceCount = matDataDirection.Count;
                            if (instanceCount > 0)
                            {
                                ...
                                if (rms[k].isCombined)
                                {
                                    for (var m = 0; m < instanceCount; m++) matrices[m] = rms[k].renderingMatrix;
                                }
                                else
                                {
                                    var objectToWorld = rms[k].transform.localToWorldMatrix;
                                    for (var m = 0; m < instanceCount; m++) matrices[m] = objectToWorld;
                                }

                                cbHighlight.DrawMeshInstanced(mesh, l, rms[k].fxMatGlow[l], 0, matrices, instanceCount,
                                    glowPropertyBlock);
                            }
                        }
                    }
                    ...
                    if (outline > 0 && outlineQuality != QualityLevel.Highest)
                    {
                        ...
                        if (outlineColorStyle == ColorStyle.Gradient)
                        {
                            ...
                            var bounds = outlineGradientInLocalSpace ? mesh.bounds : rms[k].renderer.bounds;
                            cbHighlight.SetGlobalVector(ShaderParams.OutlineVertexData,
                                new Vector4(bounds.min.y, bounds.size.y + 0.0001f, 0, 0));
                        }
                        else
                        {
                            outlineColor.a = outline * fade;
                        ...
                        if (outlineQuality.UsesMultipleOffsets())
                        {
                            ...
                            for (var o = outlineOffsetsMin; o <= outlineOffsetsMax; o++)
                            {
                                var direction = offsets[o] * (outlineWidth / 100f);
                                direction.y *= aspect;
                                if (allowGPUInstancing)
                                {
                                    matDataDirection.Add(direction);
                            ...
                            if (allowGPUInstancing)
                            {
                                ...
                                if (instanceCount > 0)
                                {
                                    ...
                                    outlinePropertyBlock.SetVectorArray(ShaderParams.OutlineDirection,
                                        matDataDirection);
                                    if (matrices == null || matrices.Length < instanceCount)
                                        matrices = new Matrix4x4[instanceCount];
                                    ...
                                    cbHighlight.DrawMeshInstanced(mesh, l, rms[k].fxMatOutline[l], 0, matrices,
                                        instanceCount, outlinePropertyBlock);
                                }
                            }
                        }
                        else
                        {
                            ...
                            if (rms[k].isSkinnedMesh)
                                ...
                                cbHighlight.DrawMesh(mesh, rms[k].transform.localToWorldMatrix, rms[k].fxMatOutline[l],
                                    l, 0);
                        }
                    }
                }
                ...
                if (targetFX)
                {
                    ...
                    if (targetFXStayDuration > 0 && Application.isPlaying)
                    {
                        ...
                        if (fadeOut > 1f) fadeOut = 1f;
                    }

                    var usesTarget = targetFXCenter != null;
                    if (fadeOut > 0 && !(targetEffectRendered && (usesTarget || targetFXUseEnclosingBounds)))
                    {
                        ...
                        var bounds = targetFXUseEnclosingBounds ? enclosingBounds : rms[k].renderer.bounds;
                        if (!targetFXScaleToRenderBounds) bounds.size = Vector3.one;

                        var scale = bounds.size;
                        var minSize = scale.x;
                        if (scale.y < minSize) minSize = scale.y;

                        if (scale.z < minSize) minSize = scale.z;

                        scale.x = scale.y = scale.z = minSize;
                        scale = Vector3.Lerp(scale * targetFXInitialScale, scale * targetFXEndScale, scaleT);
                        var center = usesTarget ? targetFXCenter.position : bounds.center;
                        ...
                        if (targetFXAlignToGround)
                        {
                            ...
                            if (Physics.Raycast(center, Vector3.down, out var groundHitInfo,
                                    targetFXGroundMaxDistance, targetFXGroundLayerMask))
                            {
                                ...
                                var m = Matrix4x4.TRS(center, rotation, scale);
                                var color = targetFXColor;
                        ...
                        else
                        {
                            ...
                            var animationRot = Quaternion.Euler(0, 0, time * targetFXRotationSpeed);
            ...
            if (useSmoothBlend && _highlighted && somePartVisible)
            {
                ...
                var smoothRTWidth = cam.pixelWidth;
                if (smoothRTWidth <= 0) smoothRTWidth = 1;

                var smoothRTHeight = cam.pixelHeight;
                if (smoothRTHeight <= 0) smoothRTHeight = 1;
#if ENABLE_VR_MODULE
                ...
                sourceDesc.colorFormat = useSmoothOutline && outlineEdgeMode == OutlineEdgeMode.Any
                ...
                var sourceTarget = new RenderTargetIdentifier(sourceRT, 0, CubemapFace.Unknown, -1);
                ...
                if (ComputeSmoothQuadMatrix(cam, enclosingBounds))
                {
                    ...
                    if (useSmoothGlow)
                    {
                        var intensity = glow * fade;
                        fxMatComposeGlow.color = new Color(glowHQColor.r * intensity, glowHQColor.g * intensity,
                            glowHQColor.b * intensity, glowHQColor.a * intensity);
                    ...
                    if (useSmoothOutline)
                    {
                        var intensity = outline * fade;
                        fxMatComposeOutline.color = new Color(outlineColor.r, outlineColor.g, outlineColor.b,
                            outlineColor.a * intensity * 10f);
        ...
        private void WorldToViewportPoint(ref Matrix4x4 m, ref Vector4 p, bool perspectiveProjection,
            ...
            float zBufferParamsW)
        {
            ...
            {
                if (usesReversedZBuffer) p.z = 1f - p.z;
        ...
        private bool ComputeSmoothQuadMatrix(Camera cam, Bounds bounds)
        {
            ...
            var shift = cam.transform.position;
            cam.transform.position = Vector3.zero;
            cam.ResetWorldToCameraMatrix();
            bounds.center -= shift;
            var res = ComputeSmoothQuadMatrixOriginShifted(cam, ref bounds, ref shift);
        ...
        private bool ComputeSmoothQuadMatrixOriginShifted(Camera cam, ref Bounds bounds, ref Vector3 shift)
        {
            ...
            var mat = GL.GetGPUProjectionMatrix(cam.projectionMatrix, false) * cam.worldToCameraMatrix;
            var min = bounds.min;
            var max = bounds.max;
            ...
            var scrMin = new Vector3(float.MaxValue, float.MaxValue, 0);
            var scrMax = new Vector3(float.MinValue, float.MinValue, 0);
            var distanceMin = float.MaxValue;
            var distanceMax = float.MinValue;
            var nearClipPlane = cam.nearClipPlane;
            var farClipPlane = cam.farClipPlane;

            float x, y, z, w;
            var isPerspectiveCamera = !cam.orthographic;
            ...
            for (var k = 0; k < 8; k++)
            {
                ...
                if (corners[k].x < scrMin.x) scrMin.x = corners[k].x;

                if (corners[k].y < scrMin.y) scrMin.y = corners[k].y;

                if (corners[k].x > scrMax.x) scrMax.x = corners[k].x;

                if (corners[k].y > scrMax.y) scrMax.y = corners[k].y;
                ...
                if (corners[k].z > distanceMax) distanceMax = corners[k].z;
            }
            ...
            if (spriteMode)
                ...
                scrMin.z = scrMax.z =
                    VRCheck.isVrRunning ? distanceMin : 0.05f + nearClipPlane; // small shift to avoid origin far issues

            if (outline > 0)
                BuildMatrix(cam, scrMin, scrMax, (int)(10 + 20 * outlineWidth + 5 * outlineDownsampling),
                    ref quadOutlineMatrix, ref shift);
            ...
            ref Vector3 shift)
        {
            ...
            // Back to world space
            var third = new Vector3(scrMax.x, scrMin.y, scrMin.z);
            scrMin = cam.ScreenToWorldPoint(scrMin);
            scrMax = cam.ScreenToWorldPoint(scrMax);
            third = cam.ScreenToWorldPoint(third);

            var width = Vector3.Distance(scrMin, third);
            var height = Vector3.Distance(scrMax, third);

            quadMatrix = Matrix4x4.TRS((scrMin + scrMax) * 0.5f + shift, cam.transform.rotation,
                new Vector3(width, height, 1f));
        }

        private void SmoothGlow(int rtWidth, int rtHeight)
        {
            var matBlur = fxMatBlurGlow;
            var glowDesc = sourceDesc;
            ...
            if (glowBlurMethod == BlurMethod.Gaussian)
            {
                ...
                for (var k = 0; k < bufferCount; k++)
                {
                    ...
                    var reducedWidth = (int)(rtWidth / reduction);
                    var reducedHeight = (int)(rtHeight / reduction);
                    if (reducedWidth <= 0) reducedWidth = 1;

                    if (reducedHeight <= 0) reducedHeight = 1;
                ...
                for (var k = 0; k < bufferCount - 1; k += 2)
                {
                    if (k == 0)
                        RenderingUtils.FullScreenBlit(cbHighlight, sourceRT, mipGlowBuffers[k + 1], fxMatBlurGlow, 0);
                    else
                        RenderingUtils.FullScreenBlit(cbHighlight, mipGlowBuffers[k], mipGlowBuffers[k + 1],
                            fxMatBlurGlow, 0);

                    RenderingUtils.FullScreenBlit(cbHighlight, mipGlowBuffers[k + 1], mipGlowBuffers[k], fxMatBlurGlow,
                        1);

                    if (k < bufferCount - 2)
                        RenderingUtils.FullScreenBlit(cbHighlight, mipGlowBuffers[k], mipGlowBuffers[k + 2],
                            fxMatBlurGlow, 2);
                }
            }
            else
            {
                ...
                var bufferCount = blurPasses;
                if (mipGlowBuffers == null || mipGlowBuffers.Length != bufferCount)
                {
                    mipGlowBuffers = new int[bufferCount];
                    for (var k = 0; k < bufferCount - 1; k++)
                        mipGlowBuffers[k] = Shader.PropertyToID("_HPSmoothGlowTemp" + k);
        ...
        {
            // Blur buffers
            var bufferCount = outlineBlurPasses * 2;
            ...
            for (var k = 0; k < bufferCount - 1; k += 2)
            {
                if (k == 0)
                    ...
                    RenderingUtils.FullScreenBlit(cbHighlight, mipOutlineBuffers[k], mipOutlineBuffers[k + 1],
                        fxMatBlurOutline, 0);

                RenderingUtils.FullScreenBlit(cbHighlight, mipOutlineBuffers[k + 1], mipOutlineBuffers[k],
                    fxMatBlurOutline, 1);

                if (k < bufferCount - 2)
                    RenderingUtils.FullScreenBlit(cbHighlight, mipOutlineBuffers[k], mipOutlineBuffers[k + 2],
                        fxMatBlurOutline, 2);
            }
        }
        ...
        private void ComposeSmoothBlend(Visibility smoothGlowVisibility, Visibility smoothOutlineVisibility)
        {
            ...
            var renderSmoothGlow = glow > 0 && glowWidth > 0 && glowQuality == QualityLevel.Highest;
            if (renderSmoothGlow)
            {
                fxMatComposeGlow.SetVector(ShaderParams.Flip,
                    VRCheck.isVrRunning && flipY ? new Vector4(1, -1) : new Vector4(0, 1));
            ...
            var renderSmoothOutline = outline > 0 && outlineWidth > 0 && outlineQuality == QualityLevel.Highest;
            if (renderSmoothOutline)
            {
                fxMatComposeOutline.SetVector(ShaderParams.Flip,
                    VRCheck.isVrRunning && flipY ? new Vector4(1, -1) : new Vector4(0, 1));
            ...
            if (renderSmoothGlow)
                for (var k = 0; k < mipGlowBuffers.Length; k++)
                    cbHighlight.ReleaseTemporaryRT(mipGlowBuffers[k]);

            if (renderSmoothOutline)
                for (var k = 0; k < mipOutlineBuffers.Length; k++)
        ...
        private void InitMaterial(ref Material material, string shaderName)
        {
            ...
            var shaderFX = Shader.Find(shaderName);
            if (shaderFX == null)
            {
        ...
        public void SetTarget(Transform transform)
        {
            ...
            if (transform != target)
            {
                if (_highlighted) ImmediateFadeOut();
        ...
        public void SetHighlighted(bool state)
        {
            if (state != _highlighted && OnObjectHighlightStateChange != null)
                if (!OnObjectHighlightStateChange(gameObject, state))
                    return;
            ...
            if (fading == FadingState.NoFading) fadeStartTime = now;

            if (state && !ignore)
            {
                if (_highlighted && fading == FadingState.NoFading) return;
                ...
                if (fadeInDuration > 0)
                {
                    ...
                    {
                        var remaining = fadeOutDuration - (now - fadeStartTime);
            ...
            else if (_highlighted)
            {
                if (fadeOutDuration > 0)
                {
                    ...
                    fading = FadingState
                        .FadingOut; // when fade out ends, highlighted will be set to false in OnRenderObject
                }
                else
                {
                    fading = FadingState.NoFading;
        ...
        private void SetupMaterial()
        {
            ...
            switch (effectGroup)
            {
                ...
                case TargetOptions.LayerInScene:
                {
                    ...
                    if (target != transform)
                    {
                        var targetEffect = target.GetComponent<HighlightEffect>();
                        if (targetEffect != null) eg = targetEffect;
                    }
        ...
        {
            if (rr == null) rr = new Renderer[0];

            if (rms == null || rms.Length < rr.Length) rms = new ModelMaterials[rr.Length];
            ...
            for (var k = 0; k < rr.Length; k++)
            {
                ...
                {
                    var otherEffect = renderer.GetComponent<HighlightEffect>();
                    if (otherEffect != null && otherEffect.enabled &&
                        otherEffect.ignore) continue; // ignore this object
                }
                ...
                var isSkinnedMesh = renderer is SkinnedMeshRenderer;
                rms[rmsCount].isSkinnedMesh = isSkinnedMesh;
                var isSpriteRenderer = renderer is SpriteRenderer;
                ...
                else if (Application.isPlaying && renderer.isPartOfStaticBatch)
                {
                    ...
                    var mc = renderer.GetComponent<MeshCollider>();
                    if (mc != null) rms[rmsCount].mesh = mc.sharedMesh;
                }
                else
                {
                ...
                if (rms[rmsCount].mesh == null) continue;
                ...
                if (!rms[rmsCount].preserveOriginalMesh)
                    if (innerGlow > 0 || (glow > 0 && glowQuality != QualityLevel.Highest) ||
                        (outline > 0 && outlineQuality != QualityLevel.Highest))
                    {
            ...
            if (spriteMode)
            {
                ...
                if (alphaCutOff <= 0) alphaCutOff = 0.5f;
            }
            else
            {
        ...
        private readonly List<Renderer> tempRR = new();
        ...
        {
            var rr = Misc.FindObjectsOfType<Renderer>();
            tempRR.Clear();
        ...
        {
            var rr = target.GetComponentsInChildren<Renderer>();
            tempRR.Clear();
            for (var i = 0; i < rr.Length; i++)
            {
                var r = rr[i];
        ...
        private void CheckGeometrySupportDependencies()
        {
            ...
            {
                var noiseTex = Resources.Load<Texture2D>("HighlightPlus/blueNoiseVL");
                fxMatGlowTemplate.SetTexture(ShaderParams.NoiseTex, noiseTex);
                if (useGPUInstancing) fxMatGlowTemplate.enableInstancing = true;
            }
        ...
        private void ConfigureOutput()
        {
            ...
            var useDepthRenderBuffer =
 colorAttachmentBuffer != BuiltinRenderTextureType.CameraTarget && depthAttachmentBuffer == BuiltinRenderTextureType.CameraTarget;
        ...
        public void UpdateVisibilityState()
        {
            ...
            if (rms == null || rms.Length != 1 || rms[0].transform != transform || rms[0].renderer == null)
                isVisible = true;
            else if (rms[0].renderer != null) isVisible = rms[0].renderer.isVisible;
        }
        ...
        public void UpdateMaterialProperties()
        {
            ...
            if (ignore) _highlighted = false;

            UpdateVisibilityState();

            maskRequired =
                (_highlighted && ((outline > 0 && outlineMaskMode != MaskMode.IgnoreMask) ||
                                  (glow > 0 && glowMaskMode != MaskMode.IgnoreMask))) ||
                seeThrough != SeeThroughMode.Never || (targetFX && targetFXAlignToGround);
            usesSeeThrough = seeThroughIntensity > 0 && (seeThrough == SeeThroughMode.AlwaysWhenOccluded ||
                                                         (seeThrough == SeeThroughMode.WhenHighlighted &&
                                                          _highlighted));
            ...
            var seeThroughTintColor = this.seeThroughTintColor;
            ...
            if (targetFXTransitionDuration <= 0) targetFXTransitionDuration = 0.0001f;

            if (targetFXStayDuration <= 0) targetFXStayDuration = 0;

            if (targetFXFadePower <= 0) targetFXFadePower = 0;

            if (seeThroughDepthOffset < 0) seeThroughDepthOffset = 0;

            if (seeThroughMaxDepth < 0) seeThroughMaxDepth = 0;

            if (seeThroughBorderWidth < 0) seeThroughBorderWidth = 0;

            if (outlineSharpness < 1f) outlineSharpness = 1f;

            shouldBakeSkinnedMesh = optimizeSkinnedMesh && ((outline > 0 && outlineQuality != QualityLevel.Highest) ||
                                                            (glow > 0 && glowQuality != QualityLevel.Highest));
            ...
            if (useSmoothBlend)
            {
                if (useSmoothGlow && useSmoothOutline) outlineVisibility = glowVisibility;
            ...
            if (useSmoothGlow)
            {
                ...
                fxMatComposeGlow.SetInt(ShaderParams.GlowStencilComp,
                    glowMaskMode != MaskMode.Stencil ? (int)CompareFunction.Always : (int)CompareFunction.NotEqual);
            ...
            if (outlineColorStyle == ColorStyle.Gradient && outlineGradient != null)
            {
                ...
                {
                    outlineGradientTex = new Texture2D(OUTLINE_GRADIENT_TEX_SIZE, 1, TextureFormat.RGBA32,
                        false, true);
                ...
                for (var k = 0; k < OUTLINE_GRADIENT_TEX_SIZE; k++)
                {
                    var t = (float)k / OUTLINE_GRADIENT_TEX_SIZE;
                    var color = outlineGradient.Evaluate(t);
            ...
            if (targetFX)
            {
                if (targetFXTexture == null) targetFXTexture = Resources.Load<Texture2D>("HighlightPlus/target");
            ...
            // Per object
            var scaledOutlineWidth = outlineQuality.UsesMultipleOffsets() ? 0f : outlineWidth / 100f;

            for (var k = 0; k < rmsCount; k++)
                if (rms[k].mesh != null)
                {
                    var renderer = rms[k].renderer;
                    if (renderer == null)
                        continue;
                    ...
                    // Per submesh
                    for (var l = 0; l < rms[k].mesh.subMeshCount; l++)
                    {
                        if (((1 << l) & subMeshMask) == 0) continue;

                        Material mat = null;
                        if (l < rendererSharedMaterials.Count) mat = rendererSharedMaterials[l];
                        ...
                        var matTextureOffset = Vector2.zero;
                        var matTextureScale = Vector2.one;
                        ...
                        {
                            matTexture =
                                mat.GetTexture(ShaderParams
                                    .BaseMap); // we don't use mainTexture because ShaderGraph doesn't inform that generic property correctly
                            hasTexture = true;
                            if (mat.HasProperty(ShaderParams.BaseMapST))
                            {
                                ...
                                var baseMapST = mat.GetVector(ShaderParams.BaseMapST);
                        ...
                        var useAlphaTest = alphaCutOff > 0 && hasTexture;

                        // Mask
                        if (rms[k].fxMatMask != null && rms[k].fxMatMask.Length > l)
                        {
                            var fxMat = rms[k].fxMatMask[l];
                            if (fxMat != null)
                            {
                                //if (hasTexture) {
                        ...
                        {
                            var fxMat = rms[k].fxMatOutline[l];
                            if (fxMat != null)
                            {
                                ...
                                fxMat.SetInt(ShaderParams.OutlineStencilComp,
                                ...
                                if (outlineColorStyle == ColorStyle.Gradient)
                                {
                                    ...
                                    fxMat.EnableKeyword(outlineGradientInLocalSpace
                                        ? ShaderParams.SKW_OUTLINE_GRADIENT_LS
                                        : ShaderParams.SKW_OUTLINE_GRADIENT_WS);
                                }
                            }
                        }
                        ...
                        {
                            var fxMat = rms[k].fxMatGlow[l];
                            if (fxMat != null)
                            {
                                fxMat.SetVector(ShaderParams.Glow2,
                                    new Vector4(outline > 0 ? outlineWidth / 100f : 0, glowAnimationSpeed,
                                        glowDithering));
                                ...
                                fxMat.SetFloat(ShaderParams.ConstantWidth, constantWidth ? 1.0f : 0);
                                fxMat.SetFloat(ShaderParams.MinimumWidth, minimumWidth);
                                fxMat.SetInt(ShaderParams.GlowStencilOp,
                                    glowBlendPasses ? (int)StencilOp.Keep : (int)StencilOp.Replace);
                                fxMat.SetInt(ShaderParams.GlowStencilComp,
                                    glowMaskMode == MaskMode.IgnoreMask
                                        ? (int)CompareFunction.Always
                                        : (int)CompareFunction.NotEqual);
                        ...
                        // See-through
                        var usesSeeThroughBorder = rms[k].fxMatSeeThroughBorder != null &&
                                                   rms[k].fxMatSeeThroughBorder.Length > l &&
                                                   seeThroughBorder * seeThroughBorderWidth > 0;
                        if (rms[k].fxMatSeeThroughInner != null && rms[k].fxMatSeeThroughInner.Length > l)
                        {
                            var fxMat = rms[k].fxMatSeeThroughInner[l];
                            if (fxMat != null)
                            {
                                ...
                                {
                                    fxMat.SetFloat(ShaderParams.SeeThroughDepthOffset,
                                        seeThroughDepthOffset > 0 ? seeThroughDepthOffset : -1);
                                    fxMat.SetFloat(ShaderParams.SeeThroughMaxDepth,
                                        seeThroughMaxDepth > 0 ? seeThroughMaxDepth : 999999);
                        ...
                        if (usesSeeThroughBorder)
                        {
                            var fxMat = rms[k].fxMatSeeThroughBorder[l];
                            if (fxMat != null)
                            {
                                fxMat.SetColor(ShaderParams.SeeThroughBorderColor,
                                    new Color(seeThroughBorderColor.r, seeThroughBorderColor.g, seeThroughBorderColor.b,
                                        seeThroughBorder));
                                fxMat.SetFloat(ShaderParams.SeeThroughBorderWidth,
                                    seeThroughBorder * seeThroughBorderWidth > 0 ? seeThroughBorderWidth / 100f : 0);
                        ...
                        {
                            var fxMat = rms[k].fxMatInnerGlow[l];
                            if (fxMat != null)
                            {
                                //if (hasTexture) {
                                fxMat.mainTexture = matTexture;
                                ...
                                fxMat.SetInt(ShaderParams.Cull, cullBackFaces ? (int)CullMode.Back : (int)CullMode.Off);
                                fxMat.SetInt(ShaderParams.InnerGlowBlendMode,
                                    innerGlowBlendMode == InnerGlowBlendMode.Additive ? 1 : 10);
                                if (useAlphaTest)
                                {
                                    fxMat.SetFloat(ShaderParams.CutOff, alphaCutOff);
                                    fxMat.EnableKeyword(ShaderParams.SKW_ALPHACLIP);
                        ...
                        {
                            var fxMat = rms[k].fxMatSolidColor[l];
                            if (fxMat != null)
                            {
                                ...
                                if ((glow > 0 && glowQuality == QualityLevel.Highest &&
                                     glowVisibility == Visibility.Normal) || (outline > 0 &&
                                                                              outlineQuality == QualityLevel.Highest &&
                                                                              outlineVisibility == Visibility.Normal))
                                {
                                    fxMat.DisableKeyword(ShaderParams.SKW_DEPTHCLIP_INV);
                                    fxMat.EnableKeyword(ShaderParams.SKW_DEPTHCLIP);
                                }
                                else if ((glow > 0 && glowQuality == QualityLevel.Highest &&
                                          glowVisibility == Visibility.OnlyWhenOccluded) || (outline > 0 &&
                                             outlineQuality == QualityLevel.Highest &&
                                             outlineVisibility == Visibility.OnlyWhenOccluded))
                                {
        ...
        {
            var mat1 = m1.renderer != null ? m1.renderer.sharedMaterial : null;
            var mat2 = m2.renderer != null ? m2.renderer.sharedMaterial : null;
            var mq1 = mat1 != null ? mat1.renderQueue : 0;
            var mq2 = mat2 != null ? mat2.renderQueue : 0;
        ...
        {
            var heading = position - cameraTransform.position;
            var distance = Vector3.Dot(heading, cameraTransform.forward);
            if (distance < cameraDistanceFadeNear) return 1f - Mathf.Min(1f, cameraDistanceFadeNear - distance);

            if (distance > cameraDistanceFadeFar) return 1f - Mathf.Min(1f, distance - cameraDistanceFadeFar);
        ...
        private void BuildQuad()
        {
            ...
            for (var i = 0; i < newNormals.Length; i++) newNormals[i] = Vector3.forward;
        ...
        {
            if (glowPasses != null)
                for (var k = 0; k < glowPasses.Length; k++)
        ...
        private static readonly Dictionary<Vector3, int> vv = new();
        private static readonly List<Material> rendererSharedMaterials = new();

        // cached meshes
        private static readonly Dictionary<int, Mesh> smoothMeshes = new();
        private static readonly Dictionary<int, Mesh> reorientedMeshes = new();
        private static readonly Dictionary<int, Mesh> combinedMeshes = new();
        ...
        private static readonly Dictionary<Mesh, int> sharedMeshUsage = new();
        private readonly List<Mesh> instancedMeshes = new();

        private void AverageNormals(int objIndex)
        {
            if (rms == null || objIndex >= rms.Length) return;
            var mesh = rms[objIndex].mesh;
            ...
            if (!smoothMeshes.TryGetValue(hashCode, out newMesh) || newMesh == null)
            {
                ...
                if (normalsCount < vertexCount) vertexCount = normalsCount;
                ...
                else
                {
                    ...
                    for (var k = 0; k < vertexCount; k++) newNormals[k] = zero;
                }

                if (matches == null || matches.Length < vertexCount) matches = new int[vertexCount];
                ...
                for (var k = 0; k < vertexCount; k++)
                {
                    int i;
                    var v = vertices[k];
                    if (!vv.TryGetValue(v, out i)) vv[v] = i = k;
        ...
        private void ReorientNormals(int objIndex)
        {
            ...
            if (!reorientedMeshes.TryGetValue(hashCode, out newMesh) || newMesh == null)
            {
                ...
                for (var k = 0; k < vertexCount; k++) mid += vertices[k];

                mid /= vertexCount;
                // Average normals
                for (var k = 0; k < vertexCount; k++) normals.Add((vertices[k] - mid).normalized);
        ...
        private void CombineMeshes()
        {
            ...
            for (var k = 0; k < rmsCount; k++)
            {
                ...
                var instanceId = rms[k].renderer.gameObject.GetInstanceID();
                if (first < 0)
                {
                    first = k;
            ...
            if (count != rmsCount) Array.Resize(ref combineInstances, count);
        ...
        {
            foreach (var mesh in combinedMeshes.Values)
                if (mesh != null)
                    DestroyImmediate(mesh);

            foreach (var mesh in smoothMeshes.Values)
                if (mesh != null)
                    DestroyImmediate(mesh);

            foreach (var mesh in reorientedMeshes.Values)
                if (mesh != null)
                    DestroyImmediate(mesh);
        }
