...
namespace HighlightPlus
{
    ...
    {
        [Tooltip("Enables highlight when pointer is over this object.")] [SerializeField]
        ...
        public static readonly List<HighlightEffect> selectedObjects = new();
        ...
        public static HighlightManager instance
        {
            get
            {
                if (_instance == null) _instance = Misc.FindObjectOfType<HighlightManager>();
        ...
        private void OnEnable()
        {
            ...
            if (baseEffect == null)
            {
                ...
                if (baseEffect == null) baseEffect = gameObject.AddComponent<HighlightEffect>();
            }
            ...
            if (raycastCamera == null)
            {
                ...
                if (raycastCamera == null) Debug.LogError("Highlight Manager: no camera found!");
            }
        ...
        private void Update()
        {
            ...
            RaycastHit hitInfo;
            if (Physics.Raycast(ray, out hitInfo, maxDistance > 0 ? maxDistance : raycastCamera.farClipPlane,
                    layerMask) && Vector3.Distance(hitInfo.point, ray.origin) >= minDistance)
            {
                var t = hitInfo.collider.transform;
                // is this object state controller by Highlight Trigger?
                var trigger = t.GetComponent<HighlightTrigger>();
                if (trigger != null) return;

                // Toggles selection
                if (InputProxy.GetMouseButtonDown(0))
                {
                    if (selectOnClick) ToggleSelection(t, !toggle);
                }
                else
                {
                    // Check if the object has a Highlight Effect
                    if (t != currentObject) SwitchesObject(t);
                }

                return;
            }
            else // check sprites
            if (Physics2D.GetRayIntersectionNonAlloc(ray, hitInfo2D,
                    maxDistance > 0 ? maxDistance : raycastCamera.farClipPlane, layerMask) > 0 &&
                Vector3.Distance(hitInfo2D[0].point, ray.origin) >= minDistance)
            {
                var t = hitInfo2D[0].collider.transform;
                // is this object state controller by Highlight Trigger?
                var trigger = t.GetComponent<HighlightTrigger>();
                if (trigger != null) return;
            ...
            // no hit
            if (selectOnClick && !keepSelection && InputProxy.GetMouseButtonDown(0) &&
        ...
        EventSystem CreateEventSystem() {
            GameObject eo =
 new GameObject("Event System created by Highlight Plus", typeof(EventSystem), typeof(UnityEngine.InputSystem.UI.InputSystemUIInputModule));
        ...
        private void VerifyHighlightStay()
        {
            ...
            if (OnObjectHighlightStay != null && !OnObjectHighlightStay(currentObject.gameObject)) SwitchesObject(null);
        }
        ...
        private void SwitchesObject(Transform newObject)
        {
            if (currentEffect != null)
            {
                if (highlightOnHover) Highlight(false);
            ...
            var ht = newObject.GetComponent<HighlightTrigger>();
            if (ht != null && ht.enabled)
                return;

            var otherEffect = newObject.GetComponent<HighlightEffect>();
            if (otherEffect == null)
            {
                // Check if there's a parent highlight effect that includes this object
                var parentEffect = newObject.GetComponentInParent<HighlightEffect>();
        ...
        private bool CanInteract()
        {
            ...
            if (Application.isMobilePlatform && InputProxy.touchCount > 0 &&
                es.IsPointerOverGameObject(InputProxy.GetFingerIdFromTouch(0)))
                return false;
        ...
        private void ToggleSelection(Transform t, bool forceSelection)
        {
            ...
            var hb = t.GetComponent<HighlightEffect>();
            if (hb == null)
            {
                var parentEffect = t.GetComponentInParent<HighlightEffect>();
            ...
            var currentState = hb.isSelected;
            var newState = forceSelection ? true : !currentState;
            ...
            if (singleSelection) internal_DeselectAll();
            ...
            if (currentEffect.isSelected)
            {
                ...
                if (!selectedObjects.Contains(currentEffect)) selectedObjects.Add(currentEffect);
            }
            else
            {
                if (currentEffect.previousSettings != null) currentEffect.previousSettings.Load(hb);

                if (selectedObjects.Contains(currentEffect)) selectedObjects.Remove(currentEffect);
            }
        ...
        public static Camera GetCamera()
        {
            ...
            if (raycastCamera == null) raycastCamera = Misc.FindObjectOfType<Camera>();
        ...
        {
            foreach (var hb in selectedObjects)
                if (hb != null && hb.gameObject != null)
                {
                    if (OnObjectUnSelected != null)
                        if (!OnObjectUnSelected(hb.gameObject))
                            continue;
        ...
        public void UnselectObject(Transform t)
        {
            ...
            if (hb.isSelected) ToggleSelection(t, false);
        }
    }
}