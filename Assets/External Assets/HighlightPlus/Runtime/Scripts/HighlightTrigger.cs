...
namespace HighlightPlus
{
    ...
    public class HighlightTrigger : MonoBehaviour
    {
        ...
        {
            if (currentTriggerMode != triggerMode) UpdateTriggers();
        }
        ...
        private void UpdateTriggers()
        {
            ...
            if (currentTriggerMode == TriggerMode.RaycastOnThisObjectAndChildren)
            {
                ...
                if (hits == null || hits.Length != MAX_RAYCAST_HITS) hits = new RaycastHit[MAX_RAYCAST_HITS];

                if (hits2D == null || hits2D.Length != MAX_RAYCAST_HITS) hits2D = new RaycastHit2D[MAX_RAYCAST_HITS];

                if (Application.isPlaying)
                {
                    StopAllCoroutines();
                    if (gameObject.activeInHierarchy) StartCoroutine(DoRayCast());
                }
            }
        }
        ...
        public void Init()
        {
            if (raycastCamera == null) raycastCamera = HighlightManager.GetCamera();

            UpdateTriggers();
            if (hb == null) hb = GetComponent<HighlightEffect>();
            ...
            while (triggerMode == TriggerMode.RaycastOnThisObjectAndChildren)
            {
                ...
                if (respectUI) {
                    ...
                    if (raycastSource == RayCastSource.MousePosition) {
                        ...
                        eventData.position =
 new Vector2(raycastCamera.pixelWidth * 0.5f, raycastCamera.pixelHeight * 0.5f);
                    }
                ...
                var isMouseButonDown = InputProxy.GetMouseButtonDown(0);
                if (hasColliders2D)
                {
                    if (maxDistance > 0)
                    ...
                    for (var k = 0; k < hitCount; k++)
                    {
                        ...
                        var theCollider = hits2D[k].collider;
                        var colliders2DCount = colliders2D.Length;
                        for (var c = 0; c < colliders2DCount; c++)
                ...
                if (hasColliders)
                {
                    ...
                    for (var k = 0; k < hitCount; k++)
                    {
                        ...
                        var theCollider = hits[k].collider;
                        var collidersCount = colliders.Length;
                        for (var c = 0; c < collidersCount; c++)
                ...
                if (!hit && currentCollider != null) SwitchCollider(null);

                if (selectOnClick && isMouseButonDown && !keepSelection && !hit)
                {
                    yield return w; // wait for other potential triggers to act
                    if (HighlightManager.lastTriggerFrame < Time.frameCount) HighlightManager.DeselectAll();
                }
 ...
 new GameObject("Event System created by Highlight Plus", typeof(EventSystem), typeof(UnityEngine.InputSystem.UI.InputSystemUIInputModule));
        ...
        private void VerifyHighlightStay()
        {
            ...
            if (OnObjectHighlightStay != null && !OnObjectHighlightStay(hb.gameObject)) SwitchCollider(null);
        }
        ...
        private void ToggleSelection()
        {
            ...
            var newState = toggle ? !hb.isSelected : true;
            if (newState)
            {
            ...
            if (singleSelection && newState) HighlightManager.DeselectAll();
